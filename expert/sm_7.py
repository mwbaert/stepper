...
#
# ANN that gives the optimal delta for a certain operating point
#
...
class DeltaModel(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(2, 512, True)
        self.fc2 = nn.Linear(512, 512, True)
        self.fc21 = nn.Linear(512, 512, True)
        self.fc3 = nn.Linear(512, 1, True)
...
class StepperEnv:
    ...
    def get_optim_delta(self, speed_lv, load_lv):
    ...
    def step(self):
        ...
        # naive control
        # global current
        # print("delta_real: {}, optim: {}".format(delta_real, self.optim))
        # if delta_real > self.optim or delta_real < 0:
        #     if current < self.current_max:
        #         current += 0.01
        # else:
        #     current -= 0.0001

        global u_a_g, u_b_g, fout_a_prev, fout_b_prev
    ...
    def get_state(self):
        optim_norm = self.get_norm(self.delta_min, self.delta_max, self.optim)
        delta_norm = self.get_norm(self.delta_min, self.delta_max, delta_real)
        return FloatTensor([float(optim_norm), float(delta_norm)]).view(1, 2)  # Floattensor size 1x1
    ...
    def get_reward(self):
        # sparse reward
        if delta_real < 150:
            # return 1 - abs(self.get_norm(30, 100, delta_real) - self.get_norm(30, 100, self.optim))
            if abs(delta_real - self.optim) < 5:
                return 1
            else:
                return -0.01
        else:
...
# write the optimal load angle to the log file
f.write('optimal load angle: {} \n'.format(stepper.get_optim_delta(speed_level, load_torque)*180))
f.flush()
...
for i_episode in range(num_episodes):
    ...
    while stepper.t < stepper.duration:
        ...
        if stepper.t > 0.05:
            # let current decrease the first 20 ms after a "stable" load angle is reached
            # if stepper.t > 0.07:  # and i_episode<20
            #     action_index = select_action(state)
            # else:
            #     action_index = LongTensor([[0]])
            action_index = select_action(state)
